 ...
 //    $_POST['world']=0;
	...
	if(count($e) != 0) {
		...
		if(isset($config['site']['newaccount_premdays']) && $config['site']['newaccount_premdays'] > 0)
		{
			...
			$reg_account->set("page_access", $local);
			$reg_account->set("email_new_time", $local);
			$reg_account->set("next_email", $local);
			$reg_account->set("last_post", $local);
			$reg_account->set("vote", $local);
			$reg_account->set("vip_time", $local);
		    $reg_account->set("guild_points", $local);
			$reg_account->set("premium_points", $local);
			$reg_account->set("page_lastday", $local);
